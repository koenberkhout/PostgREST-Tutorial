...
Install PostgreSQL on your machine. How to do this depends on your operating system. You can find the general installation instructions below, but feel free to head over to the official [download page](https://www.enterprisedb.com/downloads/postgresql) and start from there.
...
**Windows**\
Just use the official installer.

---

## A.2 Import sample data
Remember the _presidents_ database that was used in the DBS course? That's the one we will be using today, so you should already be quite familiar with it.

![Image](images/presidents-schema.png)

We use a slightly altered version of the database, that can be easily reproduced by running the following code. It's not super up-to-date, as McDonald Trump and his third lady are not yet in.
...
```
Or just download the source .sql [here](sources/presidents-custom.sql). For your convenience this database is already deployed to an Amazon AWS RDS droplet, and you can connect through the following uri: **postgres://authenticator:Fontys123@dev-postgres.cperpcjgj3fr.eu-west-1.rds.amazonaws.com:5432/presidents**. That is, _postgres://user:password@host:port/dbname_. But wait. Don't even try to mess with our database, you only have read rights.

---

## A.3 Add 'anon' role
PostgREST is going to be configured with a so-called _db-anon-role_. This role needs to exist in the PostgreSQL database, and will be the default role that is used when the API is called. You should carefully consider the access rights of this role/user, as it defines the privileges of all 'default' API users. That is, clients that are not authenticated. We'll come back to this later.

Now let's add a role _readony_ that can make SELECTs on all tables in the _api_ schema of the database:
```sql
CREATE ROLE readonly NOLOGIN;
GRANT USAGE ON SCHEMA api TO readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA api TO readonly;
```
It’s a good practice to create a dedicated role for connecting to the database, instead of using the highly privileged _postgres_ role. For this, we create the _authenticator_ role with the ability to switch to the _readonly_ role:
```sql
...
## A.4 Install PostgREST
Installing PostgREST is as easy as installing PostgreSQL, just follow the instructions below or head over to the [release page](https://github.com/PostgREST/postgrest/releases/) on Github and download the appropriate version for your operating system.

**MacOS (Homebrew)**\
`brew install postgrest`

**Linux**\
FreeBSD: `pkg install hs-postgrest`\
Arch: `sudo apt install postgresql postgresql-contrib`\
Nix: `pacman -S postgrest`

**Windows (Scoop CLI)**\
`scoop install postgrest`

If you manually downloaded the latest version for your distribution from the release page, then unzip the _.tar.xz_ file and run it with `./postgrest --help` to see usage instructions.

Just running `./postgrest` does not work. A configuration file is needed, because how else could PostgREST know for which database it needs to set up a REST API? Create a _postgrest.conf_ file and copy-paste the contents below, or download our sample [postgrest.conf](sources/postgrest.conf). Provide it as the only argument to `./postgrest`.
...
db-uri = "postgres://authenticator:Fontys123@dev-postgres.cperpcjgj3fr.eu-west-1.rds.amazonaws.com:5432/presidents"
...
db-anon-role = "readonly"

# The password for JWT authentication of the student role.
jwt-secret = "mfn4iDz7ttt7pdLCChs1mLskfPZOZ4vX"
```

Specify the db-uri with your own local database connection string, or use the database we provided: _postgres://authenticator:Fontys123@dev-postgres.cperpcjgj3fr.eu-west-1.rds.amazonaws.com:5432/presidents_. Save _postgrest.conf_ and run `./postgrest /path/to/postgrest.conf`. The API should now be accessible on _localhost:3000_.